...
void CRig::AddFitCoins(ACustomer c, int custIdx) {
    ...
    unique_lock<std::mutex> lck(mtxCoinBuffer);
...
void CRig::SolveCoin() {
    ...
    while(1) {
        if(coinBuffer . size () > 0) {
            printf("inside while in solvecoin\n");
            //vybrat
            mtxCoinBuffer.lock();
                while (coinBuffer.size () == 0){
                    printf("empty coinBuffer, waiting\n");
                    //cvEmpty.wait(lck); /* if buffer is empty, go to sleep */
                }

                printf("getting a coin to solve\n");
        ...
        //konec
        if((workCount == 0) && endFlag && (coinBuffer.size () == 0)) {
            printf("ending solveCoin\n");
            break;
        }
    }
